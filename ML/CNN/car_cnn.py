...
class VehicleClassifier(nn.Module):
    ...
    Attributes:
        ...
        gap (nn.AdaptiveAvgPool2d): Global Average Pooling 층
        classifier (nn.Sequential): 완전연결층 기반 분류기
    """
    def __init__(self, num_classes: int):
        """
        Args:
            num_classes: 분류할 차량 종류의 수
        """
        super().__init__()
        self.features = self._make_conv_layers()  # CNN 특징 추출부
        self.gap = nn.AdaptiveAvgPool2d(1)  # Global Average Pooling
        # 분류기: 완전연결층 (256->512->num_classes)
    ...
    def _make_conv_layers(self):
        """
        3개의 컨볼루션 블록으로 구성된 특징 추출기를 생성합니다.

        Returns:
            nn.Sequential: 컨볼루션 블록들의 시퀀스
        """
        return nn.Sequential(
            self._conv_block(3, 64),  # 입력 채널 3, 출력 채널 64
            self._conv_block(64, 128),  # 입력 채널 64, 출력 채널 128
            self._conv_block(128, 256),  # 입력 채널 128, 출력 채널 256
        )
